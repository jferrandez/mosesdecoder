#ifndef CLOUDLM_VIRTUAL_INTERFACE_H
#define CLOUDLM_VIRTUAL_INTERFACE_H
 ...
 *
 * For example, cdec has a WordIndex containing the entire state of a
...
class Model {
  public:
    ...
    virtual float BaseScore(const void *in_state, const WordIndex new_word, void *out_state, const cloudlm::WordIndexToString words) const = 0;

    // Requires in_state != out_state
    virtual FullScoreReturn BaseFullScore(const void *in_state, const WordIndex new_word, void *out_state, const cloudlm::WordIndexToString words) const = 0;

    // Prefer to use FullScore.  The context words should be provided in reverse order.
    virtual FullScoreReturn BaseFullScoreForgotState(const WordIndex *context_rbegin, const WordIndex *context_rend, const WordIndex new_word, void *out_state, const cloudlm::WordIndexToString words) const = 0;
...
#endif // CLOUDLM_VIRTUAL_INTERFACE_H
