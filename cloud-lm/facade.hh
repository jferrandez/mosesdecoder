#ifndef CLOUDLM_FACADE_H
#define CLOUDLM_FACADE_H
...
template <class Child, class StateT, class VocabularyT> class ModelFacade : public Model {
  public:
    ...
    FullScoreReturn BaseFullScore(const void *in_state, const WordIndex new_word, void *out_state, const cloudlm::WordIndexToString words) const {
    ...
    FullScoreReturn BaseFullScoreForgotState(const WordIndex *context_rbegin, const WordIndex *context_rend, const WordIndex new_word, void *out_state, const cloudlm::WordIndexToString words) const {
    ...
    float Score(const State &in_state, const WordIndex new_word, State &out_state, const cloudlm::WordIndexToString words) const {
      return static_cast<const Child*>(this)->FullScore(in_state, new_word, out_state, words).prob;
    }

    float BaseScore(const void *in_state, const WordIndex new_word, void *out_state, const cloudlm::WordIndexToString words) const {
