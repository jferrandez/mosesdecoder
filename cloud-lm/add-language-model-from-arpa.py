...
class Solr:
    ...
    def add_many(self, docs):
        ...
        lst = [u'<add>']
        for doc in docs:
          if type(doc) == types.TupleType:
            doc2, docattrs = doc
            self.__add(lst, doc2, docattrs)
          else:
            self.__add(lst,doc)
        lst.append(u'</add>')
        return ''.join(lst)
    ...
    def __add(self, lst, fields, docattrs = {}):
        
        doctag = u'<doc'
        for field, value in docattrs.items():
          doctag = u'{0} {1}="{2}"'.format(doctag, field, value)
...
def next_document(input):
  ...
  for line in input:
    ...
    if line == "" or line.startswith("#"):
      #print "ENTRO. ORDER {0}".format(order_ngram)
      if ngram_block == True:
        order_ngram += 1
      ngram_data = False
      ngram_block = False
      ...
      ngram_block = False
    elif line == "\\" + str(order_ngram) + "-grams:":
      #print "ORDER {0}".format(order_ngram)
      ngram_block = True
      continue
    if ngram_block:
      #print line
      parts = line.split("\t")
      # Build document
      document = {'id': document_id, 'order': order_ngram, 'gram': parts[1].decode("utf-8"), 'prob': parts[0]}
      if len(parts) == 3:
        document['backoff'] = parts[2]
      document_id += 1
      yield (document, {'boost': document_boost})
    elif ngram_data:
      ngram_counts.append(line)
  return


def help():
  sys.stderr.write("""\
Usage: {0} [options] url [input]
  Mandatory parameters:
    url                    URL where Solr is listening
  Optional parameters:
    input                  ARPA file with the n-grams, default is stdin
  Options:
    -o, --optimise         optimise index
    -q, --quiet            do not show progress
    -h, --help             shows this help\n""".format(os.path.basename(sys.argv[0])))


def add_LM(input, url, optimise, quiet):
  """Adds an alignments dictionary to a Solr instance
   :param input: CSV file with the dictionary entries
   :param url: URL of the Solr server
   :param optimise: if true, optimises Solr index
   :param quiet: if true, no info about progress of the uploading"""
  solr = SolrConnection(url)
  document_list = []
  documents_added = 0
  for document in next_document(input):
    document_list.append(document)
    if len(document_list) == 5000:
      solr.add_many(document_list, _commit = False)
      document_list = []
      documents_added += 5000
      if not quiet: sys.stderr.write("{0}K n-grams were added\n".format(documents_added / 1000))
  if len(document_list) > 0:
    solr.add_many(document_list, _commit = False)
    if not quiet: sys.stderr.write("{0} n-grams were added\n".format(documents_added + len(document_list)))
  solr.commit()
  if optimise:
    if not quiet: sys.stderr.write("Optimising ...\n")
    solr.optimize()
  solr.close()
...
def main():
  ...
  # Parse parameters
  try:
    opts, args = getopt.getopt(sys.argv[1:], "oqh", ["optimise", "quiet", "help"])
  ...
  # Process data
  add_LM(input, url, optimise, quiet)
