...
#ifndef CLOUDLM_LEFT_H
#define CLOUDLM_LEFT_H

#include "cloud-lm/max_order.hh"
...
template <class M> class RuleScore {
  public:
    ...
    void Terminal(WordIndex word, WordIndexToString words) {
      State copy(out_->right);
      FullScoreReturn ret(model_.FullScore(copy, word, out_->right, words));
    ...
    void NonTerminal(const ChartState &in, WordIndexToString words, float prob = 0.0) {
      prob_ += prob;
      ...
      if (!out_->right.length) {
        out_->right = in.right;
        if (left_done_) {
          prob_ += model_.UnRest(in.left.pointers, in.left.pointers + in.left.length, 1, words);
          return;
        }
        if (out_->left.length) {
          left_done_ = true;
      ...
      // First word
      if (ExtendLeft(in, next_use, 1, out_->right.backoff, back, words)) return;
      ...
      for (unsigned char extend_length = 2; extend_length <= in.left.length; ++extend_length) {
        if (ExtendLeft(in, next_use, extend_length, back, back2, words)) return;
        std::swap(back, back2);
      }
  ...
  private:
    bool ExtendLeft(const ChartState &in, unsigned char &next_use, unsigned char extend_length, const float *back_in, float *back_out, WordIndexToString words) {
      ...
      if (next_use != out_->right.length) {
        ...
        if (!next_use) {
          // Early exit.  
          out_->right = in.right;
          prob_ += model_.UnRest(in.left.pointers + extend_length, in.left.pointers + in.left.length, extend_length + 1, words);
          return true;
        }
      }
      // Continue scoring.  
