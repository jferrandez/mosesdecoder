...
#ifndef moses_LanguageModelCloud_h
#define moses_LanguageModelCloud_h
...
//! This will also load. Returns a templated CloudLM class
...
protected:
  ...
  /*cloudlm::WordIndex TranslateID(const Word &word) const {
    std::size_t factor = word.GetFactor(m_factorType)->GetId();
    return (factor >= m_lmIdLookup.size() ? 0 : m_lmIdLookup[factor]);
  }*/

private:
  LanguageModelCloud(const LanguageModelCloud<Model> &copy_from);

  // Convert last words of hypothesis into vocab ids, returning an end pointer.
  cloudlm::WordIndex *LastIDs(const Hypothesis &hypo, cloudlm::WordIndex *indices) const {
    cloudlm::WordIndex *index = indices;
    cloudlm::WordIndex *end = indices + m_ngram->Order() - 1;
    ...
    for (; ; ++index, --position) {
      ...
      *index = hypo.GetWord(position).GetFactor(m_factorType)->GetId();
    }
  }

  //std::vector<cloudlm::WordIndex> m_lmIdLookup;
